...
def setup_game():
    ...
    for index, row in nominators.iterrows():
        ...
        number_of_validators = np.random.poisson(
            lam=15)  # set mean to 15 since most nominators should opt for 16 validators
        ...
        while counter_selected_validators < number_of_validators:
            ...
            while validator_row_index in array_selected_validators:
                validator_row_index = int(abs(np.random.normal(loc=100, scale=200, size=1)))
            array_selected_validators[counter_selected_validators] = validators.iloc[[validator_row_index]][
                'validator_id']
...
def solve_game(nominator_dataframe, validator_dataframe, nominator_stake):
    ...
    validator_dataframe_reduced['stake_share'] = nominator_stake / validator_dataframe_reduced['stake']
    validator_dataframe_reduced['expected_return'] = (reward - (validator_dataframe_reduced['commission'] * reward)) * \
                                                     validator_dataframe_reduced['stake_share']

    validator_dataframe_reduced.sort_values('expected_return', ascending=False, inplace=True)
