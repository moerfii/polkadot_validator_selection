...
class StakingSnapshot:
    ...
    def get_specific_nominator_vote(self, address):
        targets = self.query(module='Staking',
    ...
    def get_specific_nominator_exposure(self, address):
        locked_balance = self.query(module='Balances',
    ...
    def get_voterlist_bags(self):
        return self.query_map(module='VoterList',
                              storage_function='ListBags',
                              parameters=[], block_hash=self.block_hash)

    def get_voterlist_neighbour(self, address):
        return self.query(module='VoterList',
                          storage_function='ListNodes',
                          parameters=[address], block_hash=self.block_hash)

    def get_account_indices(self):
        return self.query_map(module='Indices',
                              storage_function='Accounts',
                              parameters=[], block_hash=self.block_hash)

    def get_targets(self):
        target_dict = self.query_map(module='Staking',
                                     storage_function='ErasValidatorPrefs',
                                     parameters=[self.era], block_hash=self.block_hash)
        targets = []
...
if __name__ == "__main__":
    ...
    snapshot_instance.write_to_json("solutionstored_blocknumbers.json", block_numbers, "./block_numbers/")
    blockcounter = 0
